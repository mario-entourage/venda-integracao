...
| FR-05.9 | Reps without login. Sales reps are **not required** to use the platform. An admin can designate someone a representative without that person ever logging in: (a) **External reps** (`Novo Representante → Externo`) write a `users` doc directly with `external: true, isRepresentante: true, active: true` and a random ID — selectable in rep dropdowns immediately, no Firebase Auth identity; (b) **internal pending users** — on the Usuários page, the Representante toggle is enabled for `Pendente` (pre-registered, never-logged-in) rows; enabling it provisions an active rep `users` doc immediately (via `POST /api/admin/activate-rep`, admin-only, **not** flagged external since they can sign in later). The rep dropdown is `users` where `isRepresentante == true && active == true`; it does not depend on prior login. Once a `users` doc exists for an email, the matching pending pre-registration row is hidden to avoid duplicates, as are docs deactivated by a login merge (`mergedIntoUid`). |
| FR-05.10 | Login merge (no duplicates). If a rep designated without login (FR-05.9) later signs in, the system must **not** create a duplicate rep. On login, any other `users` doc sharing the same email is folded into the canonical UID-keyed doc: `doctors.repUserId` references are re-pointed to the surviving UID, the old doc is deactivated and marked `mergedIntoUid`, and the surviving doc inherits rep status (and UF/display name if absent). This also cleans up a duplicate left by a previous login. Best-effort and non-blocking — a merge failure must never break login. |
