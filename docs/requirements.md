...
| FR-01.2 | **Step 0 — Identification**: Select or create a client (patient), doctor, representative. Representatives are selected from users with `isRepresentante` flag (not the legacy `representantes` collection). Add products with negotiated BRL pricing via a searchable dropdown that shows full product names. Upload a prescription image via drag-and-drop. Two-step drag feedback: (1) when dragging a file anywhere on the page, the Receita zone highlights green and the Produtos area turns light red with a prohibition icon and "Não solte aqui"; (2) if the file enters the Produtos area, the warning escalates to bold repeated "NÃO!" text. |
