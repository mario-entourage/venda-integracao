...
| FR-05.9 | Reps without login. Sales reps are **not required** to use the platform. An admin can designate someone a representative without that person ever logging in: (a) **External reps** (`Novo Representante → Externo`) write a `users` doc directly with `external: true, isRepresentante: true, active: true` and a random ID — selectable in rep dropdowns immediately, no Firebase Auth identity; (b) internal pre-registrations marked as representative also surface this way once an external/pre-created record exists. The rep dropdown is `users` where `isRepresentante == true && active == true`; it does not depend on prior login. |
